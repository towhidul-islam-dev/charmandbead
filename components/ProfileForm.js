"use client";
import React, { useState, useRef, useActionState } from "react"; // 💡 Added useActionState
...
export default function ProfileForm({ user, initials }) {
  ...
  // 💡 Updated to useActionState for better React 19 compatibility
  const [state, formAction, isPending] = useActionState(async (prevState, formData) => {
    // Attach the base64 image if a new one was selected
    ...
    if (result.success) {
      // Refresh the NextAuth session with the new data
      await update({
        ...user,
        name: result.user.name,
        phone: result.user.phone,
        image: result.user.image ? `${result.user.image}?v=${Date.now()}` : "",
      });
  ...
  return (
    <form action={formAction} className="p-8 space-y-8 bg-white border border-gray-100 shadow-sm rounded-[2.5rem]">
      ...
      >
        {isPending ? <Loader2 className="animate-spin" size={20} /> : "Save Profile Changes"}
      </button>
    </form>
  );
}