...
export default function ProductPurchaseSection({ product, onVariantChange }) {
  ...
  const handleUpdateQty = (vKey, direction, moq, stock, variant) => {
    ...
    // 1. Create the new state object locally first
    const updatedQuantities = { ...quantities, [vKey]: newQty };

    // 2. Calculate the total unit count across all variants
    ...
    // 4. Send both the variant (for image/SKU) AND the total (for pricing tiers) to the parent
    if (onVariantChange) {
  ...
  const handleBulkAdd = () => {
    ...
    // });/*  */
    itemsToProcess.forEach((v) => {
  // 🟢 Dynamically determine the best possible name using your actual form fields
  let variantName = "";
  ...
  const itemImage = v.image || v.imageUrl || product.imageUrl || "/placeholder.png";

  addToCart({
    productId: product._id,
    variantId: v._id,
    name: product.name,
    variantName: variantName, // 🟢 Passed cleanly to Client & Admin Order lines
    sku: v.sku || "",
    color: v.color || "",
    size: v.size || "",
    price: v.price || product.price,
    ...
    <div className="flex flex-col gap-6 mt-10">
            ...
            {tiers
              ...
              .map((tier, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`border rounded-[1.5rem] p-4 shadow-sm flex flex-col items-center justify-center text-center transition-all duration-500 ${isAchieved ? "bg-[#EA638C] border-[#EA638C] scale-105" : "bg-white border-gray-100"}`}
                  >
                    <span
                      className={`text-[9px] font-black uppercase mb-1 ${isAchieved ? "text-white/70" : "text-gray-400"}`}
                    >
                      {tier.minQuantity}+ Pcs
                    </span>
                    <span
                      className={`text-lg font-black ${isAchieved ? "text-white" : "text-[#EA638C]"}`}
                    >
                      ৳{tier.unitPrice}
                    </span>
                    <div
                      className={`mt-1 px-2 py-0.5 text-[7px] font-black rounded-full uppercase ${isAchieved ? "bg-white text-[#EA638C]" : "bg-green-50 text-green-600"}`}
                    >
                      {isAchieved
                        ? "Tier Active"
        ...
        <div className="mx-1 p-4 bg-white border border-[#FBB6E6] rounded-[1.5rem] shadow-sm animate-bounce-subtle">
          ...
          <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(totalSelected / nextTier.minQuantity) * 100}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="sticky bottom-4 z-20 flex flex-col sm:flex-row items-center justify-between p-4 sm:p-5 bg-[#3E442B] rounded-[2rem] sm:rounded-[3rem] shadow-2xl mx-1 border border-white/10 gap-4 transition-all">
        <div className="flex flex-col items-center pl-0 sm:items-start sm:pl-4">
          <p className="text-white/50 text-[9px] font-black uppercase tracking-widest leading-none mb-1">
            Total Unit Count
          </p>
          <div className="flex items-baseline gap-2">
            <p className="text-[#FBB6E6] text-[20px] font-black italic tracking-tighter">
              {totalSelected}
              <span className="text-[12px] uppercase not-italic ml-1 text-white">
                Items
              </span>
            </p>
          </div>
    ...
    <tr className="transition-colors hover:bg-gray-50/30">
      ...
      <td className="px-6 py-4 text-center">
        <div className="flex flex-col items-center">
          <span
            className={`font-black text-[15px] italic ${liveDisplayStock < v.minOrderQuantity * 2 ? "text-[#EA638C]" : "text-[#3E442B]"}`}
          >
            {liveDisplayStock}
          </span>
          {inBagQty > 0 && (
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-[2rem] p-5 shadow-sm flex items-center justify-between active:border-[#FBB6E6] transition-all">
      <div className="flex items-center gap-4">
        ...
        <div>
          <span className="font-black text-[#3E442B] uppercase text-[14px] block leading-none mb-1">
            {v.color}
          </span>
          ...
          <div className="flex items-center gap-2 mt-2">
            <span
              className={`text-[10px] font-black ${liveDisplayStock < v.minOrderQuantity * 2 ? "text-[#EA638C]" : "text-gray-400"}`}
            >
...
function QtySelector({ v, selectionQty, liveDisplayStock, handleUpdateQty }) {
  ...
  return (
    <div className="inline-flex items-center p-1 border border-gray-200 bg-gray-50 rounded-2xl">
      <button
        onClick={() =>
          handleUpdateQty(vKey, -1, v.minOrderQuantity, v.stock, v)
        }
      ...
      <span
        className={`px-4 font-black min-w-[40px] text-center text-[15px] italic ${selectionQty > 0 ? "text-[#EA638C]" : "text-gray-300"}`}
      >
        {selectionQty}
      </span>
      <button
