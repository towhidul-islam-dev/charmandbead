...
export function getCustomerOrderEmailHtml(order = {}) {
    ...
    ? items.map((item) => `
        <tr>
          <td style="padding: 10px; border-bottom: 1px solid #eee;">
            ...
            ${item.size ? `<small style="color: #666;"> | Size: ${item.size}</small>` : ''}
          </td>
          <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: center;">${item.quantity || 1}</td>
          <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: right;">৳${item.price || 0}</td>
        </tr>
      `).join('')
    : `<tr><td colspan="3" style="padding: 10px; text-align: center; color: #777;">No items details available</td></tr>`;
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #FBB6E6; border-radius: 12px; padding: 24px; background-color: #ffffff;">
      ...
      <p style="font-size: 15px; color: #333;">Hello <strong>${order.customerName || 'Valued Customer'}</strong>,</p>
      <p style="font-size: 15px; color: #333;">We have received your order <strong>#${order.orderId || 'N/A'}</strong> and are currently processing it.</p>
      ...
      <div style="text-align: right; font-size: 16px; margin-bottom: 20px;">
        <strong>Total Amount: ৳${order.totalAmount || 0}</strong>
      </div>

      <p style="font-size: 14px; color: #555;"><strong>Payment Method:</strong> ${order.paymentMethod || 'N/A'}</p>
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #FBB6E6; border-radius: 12px; padding: 24px; background-color: #ffffff;">
      ...
      <p style="font-size: 15px; color: #333;">
        Hey <strong>Charm & Beads</strong>, you received a new order from <strong>${order.customerName || 'Customer'}</strong>.
      </p>
      ...
      <div style="background-color: #fdf5f8; border-left: 4px solid #EA638C; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 4px 0; font-size: 14px;"><strong>Name:</strong> ${order.customerName || 'N/A'}</p>
        <p style="margin: 4px 0; font-size: 14px;"><strong>Email:</strong> ${order.customerEmail || 'N/A'}</p>
      ...
      <!-- Order Details -->
      <h4 style="color: #3E442B; margin-bottom: 8px;">Order Details (#${order.orderId || 'N/A'})</h4>
...
export function getStatusUpdateEmailHtml(order = {}) {
  const { newStatus = "Updated", trackingNumber = "", orderId = "N/A", customerName = "Valued Customer", totalAmount = 0, items = [] } = order;
  ...
  const itemsHtml = itemsList.length > 0
    ? itemsList.map(item => `
        <tr>
          <td style="padding: 8px 10px; border-bottom: 1px solid #eee; font-size: 14px;">${item.name || item.productName || 'Product'}</td>
          <td style="padding: 8px 10px; border-bottom: 1px solid #eee; text-align: center; font-size: 14px;">${item.quantity || 1}</td>
          <td style="padding: 8px 10px; border-bottom: 1px solid #eee; text-align: right; font-size: 14px;">৳${item.price || 0}</td>
        </tr>
      `).join('')
    : '';
