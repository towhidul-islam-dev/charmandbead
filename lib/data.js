...
export const getProducts = cache(async (isAdmin = false, categoryId = null) => {
    try {
        ...
        // 🟢 SEO FLOW UPDATE: Filter by Category or SubCategory ID if provided
        if (categoryId) {
            query.$or = [
                { category: categoryId },
        ...
        const products = await Product.find(query)
            .select('name price stock category subCategory imageUrl createdAt hasVariants variants isArchived') 
            .sort({ createdAt: -1 }) 
            .lean(); 
