...
export const getProducts = cache(async (isAdmin = false, categoryId = null) => {
    try {
        ...
        const products = await Product.find(query)
            // 🟢 ADDED: categoryName and subCategoryName to the selection
            // 🟢 ADDED: isNewArrival and minOrderQuantity for your badges/MOQ
            .select('name price stock category categoryName subCategory subCategoryName imageUrl createdAt hasVariants variants isArchived isNewArrival minOrderQuantity') 
            .sort({ createdAt: -1 }) 
            .lean(); 
        ...
        const serializedProducts = products.map(product => ({
            ...
            // 🟢 Ensure these are passed through to the client
