...
export const getProducts = async (
    ...
    page = 1, 
    limit = 1000, 
    searchQuery = '' // 🟢 ADDED: 5th parameter for search term without altering existing defaults
) => {
    try {
        await mongodb(); 
        
        // 🟢 Ensure limit and page are treated as numbers and default to 1000 if not provided
        ...
        // 🟢 PRESERVED LOGIC: Base conditions strictly preserved
        let conditions = [
            isAdmin ? {} : { isArchived: { $ne: true } }
        ];

        // 🟢 PRESERVED LOGIC: Category filter exact structure preserved
        if (categoryId) {
            conditions.push({
                $or: [
                    { category: categoryId },
        ...
        // 🟢 ADDED: Search Filter condition (case-insensitive regex safely escaped)
        if (searchQuery && searchQuery.trim() !== '') {
            const escapedSearch = searchQuery.trim().replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&');
        ...
        const [products, totalCount] = await Promise.all([
            Product.find(query)
                ...
                .sort({ createdAt: -1 }) 
                .skip(skip)   // 🟢 Moves to the correct "slice" of data
                .limit(parsedLimit) // 🟢 Restricts to the updated limit (1000)
                .lean(),
            Product.countDocuments(query) // 🟢 Gets total for UI logic
        ]);
