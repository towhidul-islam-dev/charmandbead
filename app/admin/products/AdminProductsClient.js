...
export default function AdminProductsClient({ initialProducts }) {
  ...
  const stats = useMemo(() => {
    return {
      ...
      totalValue: products.reduce((acc, p) => acc + (Number(p.price) * (p.hasVariants ? p.variants.reduce((a, v) => a + Number(v.stock), 0) : Number(p.stock))), 0),
      lowStockCount: products.filter(p => {
        const stock = p.hasVariants ? p.variants.reduce((a, v) => a + Number(v.stock), 0) : Number(p.stock);
        return stock <= 5;
      }).length,
    };
  }, [products]);
    ...
    <div className="p-4 mx-auto md:p-8 max-w-7xl">
      ...
      <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-3">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Total Items</p>
            <p className="text-2xl italic font-black">{stats.totalItems}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 mb-6 md:grid-cols-3">
        <div className="relative md:col-span-2">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-4 bg-white border border-gray-100 rounded-2xl focus:ring-2 focus:ring-[#EA638C]/20 font-bold shadow-sm"
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        <select
          className="w-full px-4 py-4 text-[10px] font-black uppercase tracking-widest text-gray-500 bg-white border border-gray-100 rounded-2xl shadow-sm cursor-pointer"
          ...
          <table className="min-w-full divide-y divide-gray-50">
            ...
            <tbody className="bg-white divide-y divide-gray-50">
              {filteredProducts.map((product) => {
                ...
                return (
                  <tr key={product._id} className={`transition-all duration-300 ${product.isArchived ? "bg-gray-50/80 opacity-60 grayscale" : "hover:bg-gray-50/30"}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          <span className={`text-sm font-black uppercase tracking-tight ${product.isArchived ? "text-gray-400 line-through" : "text-gray-900"}`}>
                            {product.name}
                          </span>
