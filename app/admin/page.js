...
import { 
  ShoppingBag, Users, Loader2, Wallet, Truck, Percent 
} from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  // Stats Grid dynamically powered by getDashboardStats
  const stats = [
    { 
      label: "Net Revenue", 
    ...
    { 
      label: "Total Users", 
      value: (data?.totalUsers ?? 0).toLocaleString(), // 🟢 Dynamically uses backend user count
      icon: Users, 
      color: "text-[#EA638C]", 
      bg: "bg-[#EA638C]/5",
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-[#FAFAFA]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#3E442B]">
    ...
    <div className="space-y-10 duration-700 animate-in fade-in p-2 md:p-0">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, i) => (
          <div 
            key={i} 
            className="group p-8 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl hover:border-[#EA638C]/20 transition-all duration-500"
          >
            ...
            <p className="text-2xl font-black mt-1 text-[#3E442B] tracking-tighter">
              {stat.value}
            </p>
            <p className="text-[8px] font-bold text-gray-300 uppercase mt-2">
              {stat.description}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="p-8 bg-[#3E442B] rounded-[3rem] text-white shadow-2xl shadow-[#3E442B]/30 relative overflow-hidden flex flex-col justify-between">
          <div className="absolute top-0 right-0 w-48 h-48 bg-[#EA638C]/10 rounded-full -mr-24 -mt-24 blur-3xl pointer-events-none" />
          
          <div>
            <h3 className="mb-8 text-[10px] font-black tracking-[0.3em] uppercase text-white/50">
              Core Infrastructure
            </h3>
            <div className="space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-white/5">
                ...
                <div className="px-3 py-1 bg-green-500/10 text-green-400 rounded-full text-[8px] font-black uppercase tracking-tighter border border-green-500/20">
                  Stable
                </div>
              </div>
          ...
          <div className="p-6 mt-8 border bg-white/5 rounded-3xl border-white/5 backdrop-blur-sm">
            <p className="text-[8px] font-black uppercase text-[#EA638C] tracking-widest mb-2">
              Internal Note
            </p>
            <p className="text-[11px] italic leading-relaxed text-white/70">
              The Net Revenue reflects totals after MFS fees and all delivery costs have been deducted.
            </p>
          </div>
        </div>
